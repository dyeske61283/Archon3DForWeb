...
\begin{figure}[htp]
	...
	\label{fig:HP-Bonus}
\end{figure}
\footnotetext{\Quelle{vintagecomputing, \url{http://www.vintagecomputing.com/wp-content/images/archon/archon_colorchart.gif}}}
...
Als letzte Dev-Dependency ist das Paket tslint zu nennen, dass anhand einer Konfigurationsdatei den TypeScript-Code auf Lesbarkeit und Stil überprüft.
...
\stepcounter{footnote}\footnotetext{Graphviz ist eine freie Software zur Visualisierung von Graphen und Diagrammen jeglicher Art}
...
\subsubsection{Umsetzung des Layouts}
...
\begin{figure}[htp]
	\centering
	\captionsetup{justification=centering}
	\includegraphics[width=0.85\textwidth]{HTML-Page}
...
\begin{figure}[htp]
	\centering
	\captionsetup{justification=centering}
	\includegraphics[width=0.70\textwidth]{rejectedPage}
...
Die Applikation startet damit ein Server-Objekt zu erzeugen. Eine erste Implementierung dessen ist bereits in Abbildung \ref{fig:basicServerCode} zu sehen. Dort fehlen bisher die Erzeugung des Datenmodells, sowie ein erweitertes Verbindungsmanagement. Die Abbildung \ref{fig:createGameModel} zeigt die Erstellung des Datenmodells durch den Modellbuilder. Dieser Ablauf wird im Konstruktor des Server-Objektes aufgerufen, um direkt zum Start des Servers ein initialisiertes Datenmodell bereitzustellen. Zu beachten ist, dass die Initialzustände der Figuren sich in json-Dateien befinden und zur Initialisierung geladen werden. Dies bringt den Vorteil einer einfacheren Manipulation der Daten und auch ein Anpassen der Werte der Figuren während die Applikation läuft, ohne sie neustarten zu müssen.

\begin{figure}[htp]
	\centering
	\captionsetup{justification=centering}
	\includegraphics[width=0.70\textwidth]{createGameModel}
	\caption[Erzeugung Datenmodell]{Die Erstellung des Datenmodells}
	\label{fig:createGameModel}
\end{figure}

\noindent Anschließend wurde das Verbindungsmanagement implementiert, dass in Abbildung \ref{fig:connManagement} zu sehen ist. Die Abbildung zeigt die Abläufe von Anfragen eines Clients an den Webserver bis zum Spielstart. 

\begin{figure}[htp]
	\centering
	\captionsetup{justification=centering}
	\includegraphics[width=0.70\textwidth]{connManagement}
	\caption[Verwaltung Verbindungen]{Ablauf der Verbindung von Clients}
	\label{fig:connManagement}
\end{figure}

\clearpage

Zuletzt wurden dann die Logik-Objekte implementiert, eine Übersicht der Klassen ist im Anhang in Abbildung \ref{fig:overviewClasses} zu sehen. Der Fokus lag hier darauf, dass Regeln und Input des Spiels immer durch spezifische Methoden auf den Daten des Modell operieren und diese Methoden ausschließlich aus dem GameModel-Objekt heraus aufgerufen werden, als Reaktion auf Kommunikationsevents.

\subsubsection{Entwicklung des Client}

Bei der Entwicklung des Client wurde eine Startup-Routine, ähnlich der des Server, entwickelt, die in Abbildung dargestellt ist. Auch der weitere Ablauf der Entwicklung hat sich stark an dem für den Server orientiert. Es wurden also die Kommunikationsklassen ausprogrammiert und anschließend nach und nach für die Spielereignisse passende Klassen für Ansichten implementiert. Ein Teil dieser Klassen ist ebenfalls im Anhang in der Abbildung \ref{fig:overviewClasses} zu sehen.

\begin{figure}[htp]
	\centering
	\captionsetup{justification=centering}
	\includegraphics[width=0.70\textwidth]{clientStartup}
	\caption[Ablauf Client]{Ablauf des Aufbaus des Clients}
	\label{fig:clientStartup}
\end{figure}

\subsubsection{Zusammenführung der Komponenten}

Die Zusammenführung der einzelnen Komponenten wurde anhand der Reihenfolge an Ereignissen im Spiel vorgenommen. Es wurden also nacheinander, vom Aufruf der Webseite beginnend, die nötigen Events mit Methodenaufrufen gefüllt um so das Spiel aufzubauen.
...
\begin{figure}[htp]
	\centering
	\captionsetup{justification=centering}
	\includegraphics[width=0.70\textwidth]{pageWithFigures}
	\caption[Webseite Figures]{Die Webseite mit aufgestellten Figuren}
	\label{fig:pageWithFigures}
\end{figure}

Die Abbildung \ref{fig:pageWithFigures} zeigt die fertige Webseite kurz vor Spielbeginn. Die Implementierung ist an diesem Punkt abgeschlossen worden. Es wurden also Figuren umgesetzt und ein funktionierender Cursor. Außerdem können Methoden für Zauber registriert werden und die Logikklassen für fast alle Spielregeln sind fertig implementiert. Die Klassen für Ansichten sind bisher sehr rudimentär umgesetzt. Die Annahme von Eingaben und Tastaturevents ist teilweise umgesetzt worden und das Senden von Aktualisierungen zu geänderten Daten an den Client ist vollständig umgesetzt.\\
Es wurde außerdem eine Änderung in der Architektur vollzogen, die die Cursor-Klasse vollständig in den Client integriert, da die zugehörige Kommunikation für einzelne Bewegungen des Cursors über den Server als nicht wichtig bewertet wurde. Eine ähnliche Änderung gibt es für Zauber. Zauber benötigen nur eine Methode zum Ausführen des Zaubers und keinerlei visuelle Objekte, außer der Auswahl in der Oberfläche, sodass keine eigene Klasse SpellView implementiert wurde.
Ein Szenenwechsel zur Kampfszenerie ist in der View-Klasse vorgesehen und die View-Klasse stößt ein automatisches Update aller nötigen Elemente eigenständig an. 
\clearpage
...
Die anfangs definierten Anforderungen für eine Realisierung von Archon wurden teilweise erfüllt. Das Spielbrett wurde in seiner vollständigen Funktionalität implementiert und ist funktionsfähig. Die Figuren sind ebenfalls vollständig implementiert.\\
Für die Zauber, sowie das Kampfareal wurden Grundsteine gelegt, jedoch ist die Implementierung hier noch nicht abgeschlossen.

\section{Zusammenfassung}
\label{sec:Fazit}\index{Zusammenfassung}

Die erarbeitete Architektur hat sich als gute Grundlage für die Entwicklungsarbeiten erwiesen. Die Systeme von Webserver und Client, sowie der unterschiedlichen Frameworks und Hilfsmittel haben sich gut verzahnt. Die Entwicklung des Spiels an sich hat sich als sehr zeitintensiv herausgestellt.\\
Es ist also ein Webclient entstanden, der 3D-Grafik visualisiert und Daten in Echtzeit aktualisiert. Außerdem ist ein Webserver entstanden, der ein Datenmodell aufbauen kann, Verbindungsmanagement für mehrere Clients betreibt und Spielregeln und Event-Kommunikation verarbeitet.